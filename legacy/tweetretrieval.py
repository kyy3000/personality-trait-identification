...
def clean_tweets(tweet):
    ...
    #remove the colon left remain after removing mentions
    ...
    #create empty list for the filtered tweet
    filtered_tweet = []
...
#Calculate sentiment for the collective text for an account
def calculateCollectiveSentiment(txtFileLocation):
    with open(txtFileLocation, 'r', encoding='utf8') as file:
        data = file.read().replace('\n', ' ')
        filteredData = clean_tweets(data)
        blob = TextBlob(filteredData)
        return blob.sentiment.polarity

#call main method passing usernames and file path
'''
...
'''
txtLocations = ['tweets/foldablehumanAllTweets.txt', 'tweets/jennyenicholsonAllTweets.txt', 'tweets/marinscosAllTweets.txt', 'tweets/ninevoltAllTweets.txt', 'tweets/yingjuechenAllTweets.txt']

for x in range(len(txtLocations)):
    print(txtLocations[x])
    print(calculateCollectiveSentiment(txtLocations[x]))